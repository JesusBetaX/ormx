 ...
 * @author jesus
 */
public class OrmResult implements ResultSet {

  final Statement stmt;
  final ResultSet resultSet;

  public OrmResult(Statement stmt, ResultSet resultSet) throws SQLException {
    this.stmt = stmt;
    this.resultSet = resultSet;
  }
  
  public static OrmResult executeQuery(PreparedStatement ps) throws SQLException {
    ResultSet rs = ps.executeQuery();
    return new OrmResult(ps, rs);
  }
  
  public <V> List<V> list(OrmObjectAdapter<V> adapter) throws SQLException {
    try {
      return adapter.list(this);
    } finally {
      close();
    }
  }

  public <V> List<V> list(Class<V> classOf) throws SQLException {
    OrmObjectAdapter<V> adapter = OrmObjectAdapter.of(classOf);
    return list(adapter);
  }
  
  public <V> V row(OrmObjectAdapter<V> adapter) throws SQLException {
    try {
      return adapter.entity(this);
    } finally {
      close();
    }
  }
  
  public <V> V row(Class<V> classOf) throws SQLException {
    OrmObjectAdapter<V> adapter = OrmObjectAdapter.of(classOf);
    return row(adapter);
  }
  
  public <V> OrmIterator<V> it(OrmObjectAdapter<V> adapter) throws SQLException {
    return adapter.it(this);
  }
  
  public <V> OrmIterator<V> it(Class<V> classOf) throws SQLException {
    OrmObjectAdapter<V> adapter = OrmObjectAdapter.of(classOf);
    return it(adapter);
  }
  
  //  @Override public List<Map<String, Object>> resultSetToList(ResultSet rs) throws SQLException {
